...
public class ProductLibrary
{
    ...
    {
        string sqlCommandQuery = @"SELECT p.productID, p. productCode, p.productName, p.body, p.reflector, p.socket, p.paint, p.gasket, p.glass, p.weight, p.size, p.deep, CONVERT(NVARCHAR, p.imageUrl) AS 'imageUrl', p.Code, COUNT(v.code) AS 'view', p.videoPath, p.videoPath2, p.pdfPath
FROM " + dbl.tblProduct + @" p
LEFT JOIN " + dbl.tblView + @" v ON v.code = p.Code
WHERE p.productID = @productID
GROUP BY p.productID, p.productCode, p.productName, p.body, p.reflector, p.socket, p.paint, p.gasket, p.glass, p.weight, p.size, p.deep, CONVERT(NVARCHAR, p.imageUrl), p.Code, p.videoPath, p.videoPath2, p.pdfPath";
        ...
        if (dt.Rows.Count==1)
        {
            ...
            videoPath2 = cl.getString(dr["videoPath2"].ToString());
